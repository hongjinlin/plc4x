...
plc4c_return_code plc4c_s7_read_write_szl_id_serialize(plc4c_spi_write_buffer* writeBuffer, plc4c_s7_read_write_szl_id* _message) {
  ...
  // Simple Field (typeClass)
  plc4c_s7_read_write_szl_module_type_class typeClass = _message->type_class;
  _res = plc4c_s7_read_write_szl_module_type_class_serialize(writeBuffer, &typeClass);
  if(_res != OK) {
    return _res;
  }

  // Simple Field (sublistExtract)
  uint8_t sublistExtract = _message->sublist_extract;
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 4, sublistExtract);
  if(_res != OK) {
    return _res;
  }

  // Simple Field (sublistList)
  plc4c_s7_read_write_szl_sublist sublistList = _message->sublist_list;
  _res = plc4c_s7_read_write_szl_sublist_serialize(writeBuffer, &sublistList);
  if(_res != OK) {
    return _res;
  }
