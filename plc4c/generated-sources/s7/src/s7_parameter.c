...
plc4c_return_code plc4c_s7_read_write_s7_parameter_serialize(plc4c_spi_write_buffer* writeBuffer, plc4c_s7_read_write_s7_parameter* _message) {
  ...
  // Discriminator Field (parameterType)
  uint8_t parameterType = plc4c_s7_read_write_s7_parameter_get_discriminator(_message->_type).parameterType;
  plc4c_spi_write_unsigned_byte(writeBuffer, 8, parameterType);
  ...
  // Simple Field (maxAmqCaller)
  uint16_t maxAmqCaller = _message->s7_parameter_setup_communication_max_amq_caller;
  _res = plc4c_spi_write_unsigned_short(writeBuffer, 16, maxAmqCaller);
  if(_res != OK) {
    return _res;
  }

  // Simple Field (maxAmqCallee)
  uint16_t maxAmqCallee = _message->s7_parameter_setup_communication_max_amq_callee;
  _res = plc4c_spi_write_unsigned_short(writeBuffer, 16, maxAmqCallee);
  if(_res != OK) {
    return _res;
  }

  // Simple Field (pduLength)
  uint16_t pduLength = _message->s7_parameter_setup_communication_pdu_length;
  _res = plc4c_spi_write_unsigned_short(writeBuffer, 16, pduLength);
  if(_res != OK) {
    return _res;
  }
  ...
  // Implicit Field (numItems) (Used for parsing, but its value is not stored as it's implicitly given by the objects content)
  uint8_t numItems = plc4c_spi_evaluation_helper_count(_message->s7_parameter_read_var_request_items);
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 8, numItems);
  if(_res != OK) {
    return _res;
  }
  ...
  // Simple Field (numItems)
  uint8_t numItems = _message->s7_parameter_read_var_response_num_items;
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 8, numItems);
  if(_res != OK) {
    return _res;
  }
  ...
  // Implicit Field (numItems) (Used for parsing, but its value is not stored as it's implicitly given by the objects content)
  uint8_t numItems = plc4c_spi_evaluation_helper_count(_message->s7_parameter_write_var_request_items);
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 8, numItems);
  if(_res != OK) {
    return _res;
  }
  ...
  // Simple Field (numItems)
  uint8_t numItems = _message->s7_parameter_write_var_response_num_items;
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 8, numItems);
  if(_res != OK) {
    return _res;
  }
  ...
  // Implicit Field (numItems) (Used for parsing, but its value is not stored as it's implicitly given by the objects content)
  uint8_t numItems = plc4c_spi_evaluation_helper_count(_message->s7_parameter_user_data_items);
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 8, numItems);
  if(_res != OK) {
    return _res;
  }
  ...
  // Implicit Field (itemLength) (Used for parsing, but its value is not stored as it's implicitly given by the objects content)
  uint8_t itemLength = (plc4c_s7_read_write_s7_parameter_length_in_bytes(_message)) - (2);
  ...
  // Simple Field (method)
  uint8_t method = _message->s7_parameter_mode_transition_method;
  ...
  // Simple Field (cpuFunctionType)
  uint8_t cpuFunctionType = _message->s7_parameter_mode_transition_cpu_function_type;
  ...
  // Simple Field (cpuFunctionGroup)
  uint8_t cpuFunctionGroup = _message->s7_parameter_mode_transition_cpu_function_group;
  ...
  // Simple Field (currentMode)
  uint8_t currentMode = _message->s7_parameter_mode_transition_current_mode;
  _res = plc4c_spi_write_unsigned_byte(writeBuffer, 8, currentMode);
  if(_res != OK) {
    return _res;
  }

  // Simple Field (sequenceNumber)
  uint8_t sequenceNumber = _message->s7_parameter_mode_transition_sequence_number;
